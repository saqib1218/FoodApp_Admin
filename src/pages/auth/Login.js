  ...
  const handleSubmit = async (values, { setSubmitting, setFieldError }) => {
    ...
    try {
      ...
      if (result.success) {
        ...
        console.log('Login successful, permissions loaded during authentication');
        
        // Navigate immediately since permissions are loaded during login
        navigate('/');
      } else {
          ...
          <Form className="space-y-6">
            ...
            <div className="mt-6">
              <button
                ...
                className="w-full flex justify-center py-2 px-4 border border-transparent rounded-md shadow-sm text-sm font-medium text-white bg-primary-600 hover:bg-primary-700 focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-primary-600 transition-colors disabled:opacity-75"
              >
                {isLoading ? (
